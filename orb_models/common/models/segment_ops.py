...
def safe_division(numerator: torch.Tensor, denominator: torch.Tensor, segment_ids: torch.Tensor):
    """Divides logits by denominator, setting 0 where the denominator is zero."""
    result = torch.where(denominator[segment_ids] == 0, 0, numerator / denominator[segment_ids])
    return result
...
def scatter_max(
    ...
    result = result.scatter_reduce(dim, broadcasted_index, src, reduce="amax", include_self=False)
